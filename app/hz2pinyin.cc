...
int Hz2Py::pinyin(const string utf8, string &result)
{
    std::string gbk; // 机内码存储，占两个字节
    UTF82GBK(utf8, gbk);
    unsigned char ucHigh=0, ucLow=0;
    int nCode = 0;
    stringstream ss;
    string py = "";
    int length = gbk.length();

    VLOG(5) << "pinyin length: " << length;

    for (int i = 0; i < length; )
    {
        if((unsigned char)gbk[i] < 0x80)
        {
            if(isspace((unsigned char)gbk[i]))
            {
                i++;
                continue;
            }
        ...
        ucHigh = (unsigned char)gbk[i++]; // 获取机内码高字节
        ucLow  = (unsigned char)gbk[i++]; // 获取机内码低字节
...
char Hz2Py::szm(int n)
{
    /// 根据汉字区域码获取拼音声母
	graduate(0xB0A1, 0xB0C4, n, 'a');
	graduate(0XB0C5, 0XB2C0, n, 'b');
	graduate(0xB2C1, 0xB4ED, n, 'c');
	graduate(0xB4EE, 0xB6E9, n, 'd');
	graduate(0xB6EA, 0xB7A1, n, 'e');
	graduate(0xB7A2, 0xB8c0, n, 'f');
	graduate(0xB8C1, 0xB9FD, n, 'g');
	graduate(0xB9FE, 0xBBF6, n, 'h');
	graduate(0xBBF7, 0xBFA5, n, 'j');
	graduate(0xBFA6, 0xC0AB, n, 'k');
	graduate(0xC0AC, 0xC2E7, n, 'l');
	graduate(0xC2E8, 0xC4C2, n, 'm');
	graduate(0xC4C3, 0xC5B5, n, 'n');
	graduate(0xC5B6, 0xC5BD, n, 'o');
	graduate(0xC5BE, 0xC6D9, n, 'p');
	graduate(0xC6DA, 0xC8BA, n, 'q');
	graduate(0xC8BB, 0xC8F5, n, 'r');
	graduate(0xC8F6, 0xCBF0, n, 's');
	graduate(0xCBFA, 0xCDD9, n, 't');
	graduate(0xCDDA, 0xCEF3, n, 'w');
	graduate(0xCEF4, 0xD188, n, 'x');
	graduate(0xD1B9, 0xD4D0, n, 'y');
	graduate(0xD4D1, 0xD7F9, n, 'z');
	return '\0';
}

string Hz2Py::letter(int nCode)
{
	string strValue="";
	switch(nCode)
	{
